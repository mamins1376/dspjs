...
const Window = () => {
  ...
  const b1n = state === State.Running ? State.Open : State.Running;

  const b2p = useCallback({
    [State.Closed]: () => {},
    [State.Open]: () => setState(State.Closed),
    [State.Running]: () => audio.panic(),
  }[state], [state]);
  ...
  return (
    <div class="window">
      <div class="header">
        ...
        <Indicator active={state === State.Running} failure={failure} />
      </div>

      <div class="content">
        <ErrorView {...{ failure, resetError: () => setFailure("") }} />
        ...
        <div class="buttons">
          <button class={b1c} onClick={() => setState(b1n)} disabled={!!failure}>{b1l}</button>
          {b2l && !failure && <button class={b2c} onClick={b2p} >{b2l}</button>}
        </div>
...
const ErrorView = ({ failure, resetError }: { failure: string, resetError: () => void }) => {
